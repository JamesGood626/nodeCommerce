...
class Register extends React.Component {
      ...
      <div>
        <Mutation mutation={ CREATE_USER }>
          {(createUser, { data }) => (
            <div>
              <form
                onSubmit={ this.handleSubmit.bind(this, createUser) }
              >
                <h1>Register</h1>
                <input type="text" id="email" value={ this.state.email } onChange={ this.updateStateFields }/>
                <input type="text" id="password" value={ this.state.password } onChange={ this.updateStateFields }/>
  ...
  private handleSubmit = (createUser: (variables: object) => void, e: any): void => {
    e.preventDefault();
